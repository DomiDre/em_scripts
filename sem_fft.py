...
class SEM_FFT():
  ...
  def annotate_point_pair(self, ax, text, xy_start, xy_end, xycoords='data', text_offset=6, arrowprops = None, x_shift = 10):
    """
    ...
    label = ax.annotate(
      ...
      xy=xy_text,
      xycoords=xycoords,
      xytext=(text_offset * np.cos(text_angle)-x_shift, text_offset * np.sin(text_angle)),
  ...
  def pretty_plot(self, x0=200, y0=200, width=295, height=250, scaleBar=50):
    ...
    # t.remove()
    # offset = 50*self.pixels_to_nm
    # self.ax.text(
    # plotX[-1] - offset - textWidth/4,\
    # plotY[0]+height*2/20*self.pixels_to_nm+5*self.pixels_to_nm,\
    # '$'+str(scaleBar)+' \, nm$',\
    # horizontalalignment='center',
    # color='white')
    ...
    # self.ax.add_patch(
    # patches.Rectangle(
    #   (plotX[-1] - offset - textWidth/4 - scaleBar*1/2.,\
    #   plotY[0]+height*1/20*self.pixels_to_nm),   # (x,y)
    #   scaleBar,      # width
    #   height*1/40*self.pixels_to_nm,      # height
    #   color='white'
    # )
    # )
    # self.fig.tight_layout()
